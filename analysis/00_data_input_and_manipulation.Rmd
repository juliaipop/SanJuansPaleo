...
TRK_short <- TRK_wide %>% # make a new subset df with variables of interest
  ...
  mutate(chla_pheoa = chl_a_nmol_gTC + pheo_a_nmol_gTC, # add new columns
         chls = chl_a_nmol_gTC + pheo_a_nmol_gTC + chl_ap_nmol_gTC,
         ...
         greens = phaeo_b_nmol_gTC + lutein_nmol_gTC,
         cyanos = cantha_nmol_gTC + echine_nmol_gTC,
...
## TRK Interpolation 10/28
```{r}

# Dates for the samples between TRK2_058 and TRK2_082 cannot be reliably determined based on the dating model Flett created (see questions for flett doc). So, in order to estimate SOME age for plotting, assume a constant sedimentation rate between the last dated segment of the original core and the bottom of the new/original core, such that the age at 41 cm is = 114 years old. This is a somewhat reasonable extrapolation for plotting purposes based on Flett's estimation that that slice in the original core is between 89 and 160 years old. 
 ...
 TRK_dates <- all_dates %>%
        ...
        mutate(year_CE = 2025 - extrap_bottom_age_CRS) %>%
        # remove TRK2_058
        filter(!sample_ID %in% c("TRK2_058")) %>%
        # replace extrap bottom from value in reconstructed core 
        # to value in the original core.
        mutate(extrap_top = if_else(sample_ID == "TRK2_082", NA, extrap_top),
               extrap_bottom = if_else(sample_ID == "TRK2_082", 41, extrap_bottom))
    
      # NOTE 
      # AGES CORRESPOND TO EXTRAP BOTTOM
      # TRK ages come from lin-derived CRS
  
TRK_adm <- age_depth_model(TRK_dates, 
                           depth = extrap_bottom,
                           age = year_CE)
...
plot(UFM_adm,
     xlab="Depth (cm)",
     ylab="Age (year AD)",
     main="Upper Fourmile Lake") # You'll want to include these in your thesis- IAO
...
plot(LFM_adm,
     xlab="Depth (cm)",
     ylab="Age (year AD)",
     main="Lower Fourmile Lake") # You'll want to include these in your thesis- IAO
...
# this is a little circular but I actually want the TRK dfs in the same format as UFM and LFM
# copy year_CE_plot over (the long one) and name it year_plot_extrap
# trim year_CE_plot so it is empty past the end of the dated core 

TRK_wide <- TRK_wide %>%
  mutate(
    duration = if_else(year_CE_plot > 1980, year_CE_top - year_CE_bottom, NA_real_),
    year_plot_extrap = year_CE_plot,
    across(
      c(year_CE_plot, year_CE_top, year_CE_bottom),
      ~ if_else(is.na(duration), NA_real_, .)
    )
  )
