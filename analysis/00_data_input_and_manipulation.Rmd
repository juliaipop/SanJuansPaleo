...
UFM_dates <- all_dates %>%
  ...
  TRK_dates <- all_dates %>%
        ...
        filter(!sample_ID %in% c("TRK2_058", "TRK2_082"))  # remove specific samples (reassigned in the core reconstruction)
...
TRK_interp <- predict(TRK_adm, depth = seq(0.25, 11, 0.25)) %>%
  mutate(year_CE = age) %>%
...
TRK_short <- TRK_short %>%
  left_join(
    TRK_wide %>%
      ...
      select(sample_ID, year_CE_top:duration), 
    by = "sample_ID"
  )
