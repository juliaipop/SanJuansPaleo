...
## read in data
# UFM_dates <- read_excel("data/___.xlsx") 

## turn it into an age-depth model (adm) that tidypaleo recognizes
# UFM_adm <- age_depth_model(
 # UFM_dates,
 # depth = , # replace with depth column, e.g. middepth.cm
 # age = ) # "year" doesn't matter what format as long as we're consistent... so insert calculated year_ce
...
## 1.0 is the typical resolution for pigment. You can also change the resolution to 0.5 to see the dates for every sediment interval
...
TRK_short %>%
  pivot_longer(d13C.scale:perc_diatox) %>%
