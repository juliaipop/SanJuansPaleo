...
# p value too low if you change anything (add car 1 etc.)
    ...
    # residuals significant, def structured, corCAR1 helps a little byt sample suze us too small to use it.
...
```{r}
# compare data distributions
  hist(LFM_short_extrap$PI)
  hist(log(LFM_short_extrap$PI)) # log transform helped!
...
mod_PI_LFM <- gamm(PI ~ s(year_CE_plot, k = 10),
                     family=Gamma(link="log"),  
                     data = LFM_short_extrap,
                     # REML only works for gaussian data
                     method = "ML")
...
summary(mod_PI_LFM$lme) # using ML because of the Gamma family designation

resid_values_PI_LFM <- residuals(mod_PI_LFM$lme, type = "normalized")
acf(resid_values_PI_LFM) # looks good!

## summary object for use in document
PISumm_LFM <- summary(mod_PI_LFM$gam)
PISumm_LFM #Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
gam.check(mod_PI_LFM$gam)

### now transform response variable:
fam<-family(mod_PI_LFM)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# Make a 200 row list of possible year_CE_plot values from min to max.
PIYear_LFM <- with(LFM_short_extrap, 
                     data.frame(year_CE_plot = seq(min(year_CE_plot, na.rm=TRUE), 
                                                   max(year_CE_plot, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_LFM on the link scale
# if you do type = "response" se might still be on the link scale
PIYear_LFM <- cbind(PIYear_LFM,
                      data.frame(predict(mod_PI_LFM$gam, PIYear_LFM,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
PIYear_LFM <- transform(PIYear_LFM, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

PIYear_LFM$lake_ID <- 'LFM' # add a column called lake_ID filled with values "LFM"

## Plot fitted trends
ggplot(PIYear_LFM, aes(x = year_CE_plot, y = fit)) + # year_CE_plot is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_CE_plot),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = LFM_short_extrap, mapping = aes(x = year_plot_extrap, y = PI), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_PI_LFM))
fd_inc
