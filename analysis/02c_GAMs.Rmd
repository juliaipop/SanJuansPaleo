...
summary(mod_CN_LFM$lme) # using REML - good.

resid_values_perC_LFM <- residuals(mod_CN_LFM$lme, type = "normalized")
acf(resid_values_perC_LFM) # slight negative lag at 2

## summary object for use in document
perCSumm_LFM <- summary(mod_CN_LFM$gam)
perCSumm_LFM #Gives you the P values, degrees of freedom...
...
# Second, if you're only concerned about the response, include "response" in type of predict()???C
...
### this calculates on the link scale 
CNYear_LFM <- transform(CNYear_LFM, upper = fit + (2 * se.fit), lower = fit - (2 * se.fit))
CNYear_LFM$lake_ID <- 'LFM'

## Plot fitted trends
ggplot(CNYear_LFM, aes(x = year_CE_plot, y = fit)) +
  geom_ribbon(aes(ymin = (lower),
                  ymax = (upper), x = year_CE_plot),
              alpha = 0.2, inherit.aes = FALSE, fill = "black") +
