...
# Use UFM_short. Columns are the same (year_CE_plot for interp and year_plot_extrap for extrapolated)
```
...
# Our dataset is still small: rule of thumb is to not let k approach n
    # how much data are we working with? 
    sum(!is.na(UFM_short_extrap$year_CE_plot)) # only 5 observations. Can you even run a gam here...?
    sum(!is.na(UFM_short_extrap$year_plot_extrap)) # 43 total. 

# try with and without corCAR1, then examine the residuals. 
mod_d15N_UFM <- gamm(d15N ~ s(year_CE_plot, k = 5), 
                     family = gaussian(link = "identity"), 
