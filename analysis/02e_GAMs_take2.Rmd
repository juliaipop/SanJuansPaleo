...
# View(select(TRK_short, c(depth_plot, "C.N", C.Nmolar, year_plot_extrap, duration)))
```
...
# run the model without transformation and see how model checks look
  mod_perC_TRK_ID <- gam(percent.C ~ s(year_plot_extrap, k = 10), 
                     family= gaussian(link = "identity"),  
                     data = TRK_short,
                     method = "REML")
  summary(mod_perC_TRK_ID)

  par(mfrow = c(2, 2))   # Set up a 2x2 grid
  gam.check(mod_perC_TRK_ID)
  par(mfrow = c(1, 1)) 
  
# compared to the gam.chcek below this is definitely worse.
...
# check later how weights, corCAR1 compare. 
  ...
  # data is right skewed, log transform makes it worse? in this case it looks a little better?
    
# attempt with transformation of the data first?
  
TRK_short$C.N_rec <- (1/TRK_short$C.N)
  hist(1/TRK_short$C.N)
  hist(TRK_short$C.N_rec) # same, good

mod_CN_TRK_rec <- gam((1/C.N) ~ s(year_plot_extrap, k = 20),
                      family = gaussian(link="identity"),
                      data = TRK_short,
                      method = "REML")
mod_CN_TRK_rec2 <- gam(C.N_rec ~ s(year_plot_extrap, k = 20),
                      family = gaussian(link="identity"),
                      data = TRK_short,
                      method = "REML")

# summary(mod_CN_TRK)
# summary(mod_CN_TRK_rec2)

# AIC(mod_CN_TRK, mod_CN_TRK_rec2) 
# comment out before leaving!
# both do the same thing, let's check normality assumptions

par(mfrow = c(2, 2))   # Set up a 2x2 grid
# gam.check(mod_CN_TRK)
# gam.check(mod_CN_TRK_ID)
gam.check(mod_CN_TRK_rec)
gam.check(mod_CN_TRK_rec2)
par(mfrow = c(1, 1)) 

# mod_CN_TRK <- gam(C.N ~ s(year_plot_extrap, k = 10), 
#                      family = gaussian(link = "identity"), 
#                      data = TRK_short,
#                    #  correlation = corCAR1(form = ~ year_plot_extrap),
#                      method = "REML")

# See model output with raw data
mod_CN_TRK_ID <- gam(C.N ~ s(year_plot_extrap, k = 20),
                     family=gaussian(link="identity"),  
                     data = TRK_short,
                     method = "REML")
summary(mod_CN_TRK_ID)
gam.check(mod_CN_TRK_ID)
# this is def worse than Bella's version below...
...
# p value too low if you change anything (add car 1 etc.)
    ...
    resid_values_CN_TRK <- residuals(mod_CN_TRK, type = "response")
...
CNYear_TRK <- cbind(CNYear_TRK,
                      data.frame(predict(mod_CN_TRK, CNYear_TRK,
                                         type="response", se.fit = TRUE)))
...
# this column name is annoying so change it to match LFM
  # names(TRK_short)
  # TRK_short <- rename(TRK_short, C.Nmolar = `C:N`)

# check later how weights, corCAR1 compare. 
    
  hist(TRK_short$C.Nmolar)
  hist(log(TRK_short$C.Nmolar))
  hist(-log(TRK_short$C.Nmolar))
...
TRK_short$C.Nmolar_rec <- (1/TRK_short$C.Nmolar)
  hist(1/TRK_short$C.Nmolar)
  hist(TRK_short$C.Nmolar_rec) # same, good

mod_C.Nmolar_TRK_rec <- gam((1/C.Nmolar) ~ s(year_plot_extrap, k = 20),
                      family = gaussian(link="identity"),
                      data = TRK_short,
                      method = "REML")
mod_C.Nmolar_TRK_rec2 <- gam(C.Nmolar_rec ~ s(year_plot_extrap, k = 20),
                      family = gaussian(link="identity"),
                      data = TRK_short,
                      method = "REML")

# summary(mod_C.Nmolar_TRK)
# summary(mod_C.Nmolar_TRK_rec2)

# AIC(mod_C.Nmolar_TRK, mod_C.Nmolar_TRK_rec2) 
# comment out before leaving!
# both do the same thing, let's check normality assumptions

par(mfrow = c(2, 2))   # Set up a 2x2 grid
# gam.check(mod_C.Nmolar_TRK)
# gam.check(mod_C.Nmolar_TRK_ID)
gam.check(mod_C.Nmolar_TRK_rec)
gam.check(mod_C.Nmolar_TRK_rec2)
par(mfrow = c(1, 1)) 

# mod_C.Nmolar_TRK <- gam(C.Nmolar ~ s(year_plot_extrap, k = 10), 
#                      family = gaussian(link = "identity"), 
#                      data = TRK_short,
#                    #  correlation = corCAR1(form = ~ year_plot_extrap),
#                      method = "REML")
...
# See model output with raw data
mod_C.Nmolar_TRK_ID <- gam(C.Nmolar ~ s(year_plot_extrap, k = 20),
                     family=gaussian(link="identity"),  
                     data = TRK_short,
                     method = "REML")
summary(mod_C.Nmolar_TRK_ID)
gam.check(mod_C.Nmolar_TRK_ID)
# this is def worse than Bella's version below...
  
# IAO - version 2 with Gamma because data are left-skewed, all positive
mod_C.Nmolar_TRK <- gam(C.Nmolar ~ s(year_plot_extrap, k = 20),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")

summary(mod_C.Nmolar_TRK) #$dev.expl  # deviance explained
gam.check(mod_C.Nmolar_TRK)

AIC(mod_C.Nmolar_TRK, mod_C.Nmolar_TRK_ID)
summary(mod_C.Nmolar_TRK)
...
# p value too low if you change anything (add car 1 etc.)
    ...
    resid_values_C.Nmolar_TRK <- residuals(mod_C.Nmolar_TRK, type = "response")
    acf(resid_values_C.Nmolar_TRK) 
    # residuals significant, def structured, corCAR1 helps a little but sample size is too small to use it.
    
## summary object for use in document
C.NmolarSumm_TRK <- summary(mod_C.Nmolar_TRK)
C.NmolarSumm_TRK #Gives you the P values, degrees of freedom...
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
C.NmolarYear_TRK <- with(TRK_short, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))

# predict fit (response variable) and se for perCYear_TRK on the response scale
C.NmolarYear_TRK <- cbind(C.NmolarYear_TRK,
                      data.frame(predict(mod_C.Nmolar_TRK, C.NmolarYear_TRK,
                                         type="response", se.fit = TRUE)))

# calculate 95% CIs, on the response scale 

C.NmolarYear_TRK <- transform(C.NmolarYear_TRK,
  upper = fit + 2 * se.fit,
  lower = fit - 2 * se.fit)

C.NmolarYear_TRK$lake_ID <- 'TRK' # add a column called lake_ID filled with values "TRK"
     
## Plot fitted trends
ggplot(C.NmolarYear_TRK, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = TRK_short, mapping = aes(x = year_plot_extrap, y = C.Nmolar), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_C.Nmolar_TRK))
fd_inc
...
pred <- left_join(C.NmolarYear_TRK,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
TRK_C.Nmolar <- ggplot(TRK_short) +
  #Add a box to show the reader the time period for interpretation
    ...
    fill = "blue",
    inherit.aes = FALSE) + 
  geom_point(aes(year_plot_extrap, C.Nmolar)) +
  geom_ribbon(data = pred,
              aes(x = year_plot_extrap, ymin = lower, ymax = upper),
              alpha = 0.2) +
  geom_line(data = pred,
...
ggsave("figures/TRK_C.Nmolar.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
mod_pheoa_TRK <- gam(pheo_a_nmol_gTC ~ s(year_plot_extrap, k = 20),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")


summary(mod_pheoa_TRK) 
...
# compare data distributions
  hist(TRK_short$chla_pheoa)
  hist(log(TRK_short$chla_pheoa)) # better!

    # check later how weights, corCAR1 compare. 
    
 mod_chla_pheoa_TRK <- gam(chla_pheoa ~ s(year_plot_extrap, k = 20),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")

summary(mod_chla_pheoa_TRK)
gam.check(mod_chla_pheoa_TRK)
 
summary(mod_chla_pheoa_TRK) 

resid_values_chla_pheoa_TRK <- residuals(mod_chla_pheoa_TRK, type = "response")
acf(resid_values_chla_pheoa_TRK) 

## summary object for use in document
chla_pheoaSumm_TRK <- summary(mod_chla_pheoa_TRK)
chla_pheoaSumm_TRK # Gives you the P values, degrees of freedom...
...
### now transform response variable:
fam<-family(mod_chla_pheoa_TRK)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 

plot(mod_chla_pheoa_TRK)
### NOW PLOT! ###
N <- 300    # number of points at which to evaluate the splines

# Make a 200 row list of possible year_plot_extrap values from min to max.
chla_pheoaYear_TRK <- with(TRK_short, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_TRK on the link scale
# if you do type = "response" se might still be on the link scale
chla_pheoaYear_TRK <- cbind(chla_pheoaYear_TRK,
                      data.frame(predict(mod_chla_pheoa_TRK, chla_pheoaYear_TRK,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
chla_pheoaYear_TRK <- transform(chla_pheoaYear_TRK, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

chla_pheoaYear_TRK$lake_ID <- 'TRK' # add a column called lake_ID filled with values "TRK"

## Plot fitted trends
ggplot(chla_pheoaYear_TRK, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = TRK_short, mapping = aes(x = year_plot_extrap, y = chla_pheoa), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_chla_pheoa_TRK))
fd_inc
...
pred <- left_join(chla_pheoaYear_TRK,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
TRK_chla_pheoa <- ggplot(TRK_short) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
    ...
    fill = "blue",
    inherit.aes = FALSE) + 
  geom_point(aes(year_plot_extrap, chla_pheoa)) +
  geom_ribbon(data = pred,
              aes(x = year_plot_extrap, ymin = lower, ymax = upper),
              alpha = 0.2) +
  geom_line(data = pred,
...
#            linewidth = 1) +
  scale_color_manual(values = c("sig. inc." = "#CC9933",
                     ...
                     name ="Statistic") +
  # scale_x_continuous(breaks=seq(1880,2025,by=25))+
  ylab(expression("chlorophyll and pheophytin a ("*nmol~g^-1~"total carbon"*")")) + 
  xlab(expression(paste("Year (", italic("CE"), ")"))) +
  ggtitle("TRK")
TRK_chla_pheoa

ggsave("figures/TRK_chla_pheoa.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
# compare data distributions
  hist(TRK_short$chls)
  hist(log(TRK_short$chls)) # better!

    # check later how weights, corCAR1 compare. 
    
 mod_chls_TRK <- gam(chls ~ s(year_plot_extrap, k = 20),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")


summary(mod_chls_TRK) 

resid_values_chls_TRK <- residuals(mod_chls_TRK, type = "response")
acf(resid_values_chls_TRK) 

## summary object for use in document
chlsSumm_TRK <- summary(mod_chls_TRK)
chlsSumm_TRK # Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_chls_TRK)
par(mfrow = c(1, 1)) 

### now transform response variable:
fam<-family(mod_chls_TRK)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# predict fit (response variable) and se for perCYear_TRK on the link scale
# if you do type = "response" se might still be on the link scale
chlsYear_TRK <- cbind(chlsYear_TRK,
                      data.frame(predict(mod_chls_TRK, chlsYear_TRK,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
chlsYear_TRK <- transform(chlsYear_TRK, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

chlsYear_TRK$lake_ID <- 'TRK' # add a column called lake_ID filled with values "TRK"

## Plot fitted trends
ggplot(chlsYear_TRK, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = TRK_short, mapping = aes(x = year_plot_extrap, y = chls), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_chls_TRK))
fd_inc
...
TRK_chls_ap <- ggplot(TRK_short) +
  #Add a box to show the reader the time period for interpretation
    ...
    fill = "blue",
    inherit.aes = FALSE) + 
  geom_point(aes(year_plot_extrap, chls)) +
  geom_ribbon(data = pred,
              aes(x = year_plot_extrap, ymin = lower, ymax = upper),
              alpha = 0.2) +
  geom_line(data = pred,
...
#            linewidth = 1) +
  scale_color_manual(values = c("sig. inc." = "#CC9933",
                     ...
                     name ="Statistic") +
  # scale_x_continuous(breaks=seq(1880,2025,by=25))+
  ylab(expression("chlorophylls and pheophytin a ("*nmol~g^-1~"total carbon"*")")) + 
  xlab(expression(paste("Year (", italic("CE"), ")"))) +
  ggtitle("TRK")
TRK_chls_ap

ggsave("figures/TRK_chls_ap.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
# if PI > 1 filter out pheophytin a 
TRK_short_fil <- TRK_short %>%
  filter(!PI>1)
    
mod_pheoa_fil_TRK <- gamm(pheo_a_nmol_gTC ~ s(year_plot_extrap, k = 10),
                     family=Gamma(link="log"),  
                     data = TRK_short_fil,
                     method = "REML")

summary(mod_pheoa_fil_TRK$lme) 

resid_values_pheoa_fil_TRK <- residuals(mod_pheoa_fil_TRK$lme, type = "normalized")
acf(resid_values_pheoa_fil_TRK) # looks good!

## summary object for use in document
pheoaSumm_fil_TRK <- summary(mod_pheoa_fil_TRK$gam)
pheoaSumm_fil_TRK #Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_pheoa_fil_TRK$gam)
par(mfrow = c(1, 1)) 

### now transform response variable:
fam<-family(mod_pheoa_fil_TRK)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
pheoaYear_fil_TRK <- with(TRK_short_fil, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_TRK on the link scale
# if you do type = "response" se might still be on the link scale
pheoaYear_fil_TRK <- cbind(pheoaYear_fil_TRK,
                      data.frame(predict(mod_pheoa_fil_TRK$gam, pheoaYear_fil_TRK,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
pheoaYear_fil_TRK <- transform(pheoaYear_fil_TRK, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

pheoaYear_fil_TRK$lake_ID <- 'TRK' # add a column called lake_ID filled with values "TRK"

## Plot fitted trends
ggplot(pheoaYear_fil_TRK, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = TRK_short_fil, mapping = aes(x = year_plot_extrap, y = pheo_a_nmol_gTC), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_pheoa_fil_TRK))
fd_inc

#Add years for plotting
years <- with(TRK_short_fil,
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), max(year_plot_extrap, na.rm=TRUE), length.out = 200)))
...
pred <- left_join(pheoaYear_fil_TRK,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
###Greens - see if cor structure helps
```{r}
mod_greens_TRK <- gamm(greens ~ s(year_plot_extrap, k = 10),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")

mod_greens_TRK_cor <- gamm(greens ~ s(year_plot_extrap, k = 10),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")
                  
AIC(mod_greens_TRK$lme, mod_greens_TRK_cor$lme)

resid_greens_TRK <- residuals(mod_greens_TRK$lme, type = "normalized")
resid_greens_TRK_cor <- residuals(mod_greens_TRK_cor$lme, type = "normalized")


acf(resid_greens_TRK) 
acf(resid_greens_TRK_cor)

# slight positive correlation at lag 7 - not something corCAR1 is meant to handle, 
# and either way neither corstructure helps resolve this. 
...
###Master Preservation Index
```{r}
TRK_short_mchl <- TRK_short %>% drop_na(year_plot_extrap, master_chl)
...
TRK_master_chl <- ggplot(TRK_short_mchl) +
   #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
    ...
    fill = "blue",
    inherit.aes = FALSE) + 
  geom_point(aes(year_plot_extrap, master_chl)) +
#  geom_ribbon(data = pred,
#              aes(x = year_plot_extrap, ymin = lower, ymax = upper),
#              alpha = 0.2) +
#  geom_line(data = pred,
...
#                     name ="Statistic") +
  # scale_x_continuous(breaks=seq(1880,2025,by=25))+
  # Added for non gam version - connect points with a line
  geom_line(aes(x = year_plot_extrap, y = master_chl), 
            linewidth = 0.8, color = "black") +
  # Add dashed reference line at y = 1
  geom_hline(yintercept = 1, linetype = "dashed", color = "red", linewidth = 0.7) +
  ylab(expression("Master Preservation Index")) + 
  xlab(expression(paste("Year (", italic("CE"), ")"))) +
  ggtitle("TRK")
TRK_master_chl

ggsave("figures/TRK_masterPI.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
# Our dataset is still small: rule of thumb is to not let k approach n
    # how much data are we working with? 
    sum(!is.na(LFM_short_extrap$year_plot_extrap)) # 18 observations. 

# Weights didn't help at all above. Model here without... 
    
# New edits 11/10: 
  # using gam instead of gamm to get the variance explained, but gamm doesn't 
  # run models with the correlation structure... 

  # so, just double check again how gamm functions w and without the correlation 
  # structure on the new data: 

mod_d15N_LFM <- gamm(d15N ~ s(year_plot_extrap, k = 10),
                    family = gaussian(link = "identity"), 
                    data = LFM_short_extrap,
                    method = "REML")


mod_d15N_LFM_CAR1 <- gamm(d15N ~ s(year_plot_extrap, k = 10), 
                     family = gaussian(link = "identity"), 
                     data = LFM_short_extrap,
                     correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")

summary(mod_d15N_LFM_CAR1$gam)
summary(mod_d15N_LFM_CAR1$lme) # using REML, corellation structure phi = 0.29

summary(mod_d15N_LFM$gam) 
summary(mod_d15N_LFM$lme) 

# need $lme to get acf to work
...
acf(resid_values)
acf(resid_values_CAR1) # residuals are the same with and without CAR1

# examine again using AIC:
AIC(mod_d15N_LFM$lme)   # without CAR(1): -0.57
AIC(mod_d15N_LFM_CAR1$lme) # with CAR(1): 1.42

# lower AIC = better? better without CAR(1).
...
gam.check(mod_d15N_LFM_CAR1$gam)
par(mfrow = c(1, 1)) # both madels are identical 


# now run/plot as a gam
mod_d15N_LFM <- gam(d15N ~ s(year_plot_extrap, k = 10),
                    family = gaussian(link = "identity"), 
                    data = LFM_short_extrap,
                    method = "REML")

summary(mod_d15N_LFM)

# type = response calculates residuals on the identity scale
# if I wanted to see whether the transformation function was appropriate I could  
# analyze residuals on the link scale. here this doesn't matter because we are 
# working on the identity scale anyway but leave the code in for when we do transoformations. 
resid_values <- residuals(mod_d15N_LFM, type = "response")  
acf(resid_values) # slight neg autocorellation at lag 1
...
# Second, if you're only concerned about the response (and not the resp on the link scale),
# include "response" in type of predict()
d15NYear_LFM <- with(LFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), max(year_plot_extrap, na.rm=TRUE), length.out = 200)))

d15NYear_LFM <- cbind(d15NYear_LFM,
                      data.frame(predict(mod_d15N_LFM, d15NYear_LFM,
                                         type="response", se.fit = TRUE)))
...
## Plot fitted trends
ggplot(d15NYear_LFM, aes(x = year_plot_extrap, y = fit)) +
  geom_ribbon(aes(ymin = (lower),
                  ymax = (upper), x = year_plot_extrap),
              alpha = 0.2, inherit.aes = FALSE, fill = "black") +
...
LFM_perC <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
LFM_perN <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
  ...
  ylab("Total N (%)") +
  xlab(expression(paste("Year (", italic("CE"), ")"))) +
  geom_point(aes(year_plot_extrap, percent.N), size=0.5) +
  ggtitle("LFM")
LFM_perN
...
mod_CN_LFM_gamma <- gam(C.N ~ s(year_plot_extrap, k = 10), 
                     family = Gamma(link = "log"), 
                     data = LFM_short_extrap,
                   #  correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")

AIC(mod_CN_LFM, mod_CN_LFM_gamma)

summary(mod_CN_LFM)
summary(mod_CN_LFM_gamma)

par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_CN_LFM)
gam.check(mod_CN_LFM_gamma)
par(mfrow = c(1, 1)) 

# better without link function
...
# check later how weights, corCAR1 compare. 
  ...
  hist(LFM_short_extrap$C.Nmolar)
  hist(log(LFM_short_extrap$C.Nmolar))
  # data is left skewed, log transform makes it worse
    
mod_C.Nmolar_LFM <- gam(C.Nmolar ~ s(year_plot_extrap, k = 10), 
                     family = gaussian(link = "identity"), 
                     data = LFM_short_extrap,
                   #  correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")

mod_C.Nmolar_LFM_gamma <- gam(C.Nmolar ~ s(year_plot_extrap, k = 10), 
                     family = Gamma(link = "log"), 
                     data = LFM_short_extrap,
                   #  correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")

AIC(mod_C.Nmolar_LFM, mod_C.Nmolar_LFM_gamma)

summary(mod_C.Nmolar_LFM)
summary(mod_C.Nmolar_LFM_gamma)

par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_C.Nmolar_LFM)
gam.check(mod_C.Nmolar_LFM_gamma)
par(mfrow = c(1, 1)) 
...
# p value too low if you change anything (add car 1 etc.)
    ...
    resid_values_C.Nmolar_LFM <- residuals(mod_C.Nmolar_LFM, type = "response")
    acf(resid_values_C.Nmolar_LFM) 
    # residuals significant, def structured, corCAR1 helps a little bit but sample size is too small to use it.
    
## summary object for use in document
C.NmolarSumm_LFM <- summary(mod_C.Nmolar_LFM)
C.NmolarSumm_LFM #Gives you the P values, degrees of freedom...
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
C.NmolarYear_LFM <- with(LFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))

# predict fit (response variable) and se for perCYear_LFM on the response scale
C.NmolarYear_LFM <- cbind(C.NmolarYear_LFM,
                      data.frame(predict(mod_C.Nmolar_LFM, C.NmolarYear_LFM,
                                         type="response", se.fit = TRUE)))

# calculate 95% CIs, on the response scale 

C.NmolarYear_LFM <- transform(C.NmolarYear_LFM,
  upper = fit + 2 * se.fit,
  lower = fit - 2 * se.fit)

C.NmolarYear_LFM$lake_ID <- 'LFM' # add a column called lake_ID filled with values "LFM"
     
## Plot fitted trends
ggplot(C.NmolarYear_LFM, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = LFM_short_extrap, mapping = aes(x = year_plot_extrap, y = C.Nmolar), inherit.aes = FALSE) +
      geom_line() 
...
fd_inc = confint(fderiv(mod_C.Nmolar_LFM))
fd_inc
...
plot(mod_C.Nmolar_LFM, rug=TRUE, shade=TRUE)
...
pred <- left_join(C.NmolarYear_LFM,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
LFM_C.Nmolar <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/LFM_C.Nmolar.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
mod_pheoa_LFM_gaussian <- gam(pheo_a_nmol_gTC ~ s(year_plot_extrap, k = 10),
                     family=gaussian(link="identity"),  
                     data = LFM_short_extrap,
                     method = "REML")

summary(mod_pheoa_LFM) 
summary(mod_pheoa_LFM_gaussian)



AIC(mod_pheoa_LFM, mod_pheoa_LFM_gaussian)
...
gam.check(mod_pheoa_LFM_gaussian)
par(mfrow = c(1, 1)) 
...
LFM_pheo_a <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
```{r}
# compare data distributions
  hist(LFM_short_extrap$chla_pheoa)
  hist(log(LFM_short_extrap$chla_pheoa))
    
mod_chla_pheoa_LFM <- gam(chla_pheoa ~ s(year_plot_extrap, k = 10),
                     family=Gamma(link="log"),  
                     data = LFM_short_extrap,
                     method = "REML")

mod_chla_pheoa_LFM_gaussian <- gam(chla_pheoa ~ s(year_plot_extrap, k = 10),
                     family=gaussian(link="identity"),  
                     data = LFM_short_extrap,
                     method = "REML")

summary(mod_chla_pheoa_LFM) 
summary(mod_chla_pheoa_LFM_gaussian)



AIC(mod_chla_pheoa_LFM, mod_chla_pheoa_LFM_gaussian)

resid_values_chla_pheoa_LFM <- residuals(mod_chla_pheoa_LFM, type = "response")
acf(resid_values_chla_pheoa_LFM) # looks good!

## summary object for use in document
chla_pheoaSumm_LFM <- summary(mod_chla_pheoa_LFM)
chla_pheoaSumm_LFM #Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_chla_pheoa_LFM)
gam.check(mod_chla_pheoa_LFM_gaussian)
par(mfrow = c(1, 1)) 

### now transform response variable:
fam<-family(mod_chla_pheoa_LFM)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
chla_pheoaYear_LFM <- with(LFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_LFM on the link scale
# if you do type = "response" se might still be on the link scale
chla_pheoaYear_LFM <- cbind(chla_pheoaYear_LFM,
                      data.frame(predict(mod_chla_pheoa_LFM, chla_pheoaYear_LFM,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
chla_pheoaYear_LFM <- transform(chla_pheoaYear_LFM, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

chla_pheoaYear_LFM$lake_ID <- 'LFM' # add a column called lake_ID filled with values "LFM"

## Plot fitted trends
ggplot(chla_pheoaYear_LFM, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = LFM_short_extrap, mapping = aes(x = year_plot_extrap, y = chla_pheoa), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_chla_pheoa_LFM))
fd_inc
...
pred <- left_join(chla_pheoaYear_LFM,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
LFM_chla_pheoa <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/LFM_chla_pheoa.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
```{r}
# compare data distributions
  hist(LFM_short_extrap$chls)
  hist(log(LFM_short_extrap$chls))
    
mod_chls_LFM <- gam(chls ~ s(year_plot_extrap, k = 10),
                     family=Gamma(link="log"),  
                     data = LFM_short_extrap,
                     method = "REML")

mod_chls_LFM_gaussian <- gam(chls ~ s(year_plot_extrap, k = 10),
                     family=gaussian(link="identity"),  
                     data = LFM_short_extrap,
                     method = "REML")

summary(mod_chls_LFM) 
summary(mod_chls_LFM_gaussian)



AIC(mod_chls_LFM, mod_chls_LFM_gaussian)

resid_values_chls_LFM <- residuals(mod_chls_LFM, type = "response")
acf(resid_values_chls_LFM) # looks good!

## summary object for use in document
chlsSumm_LFM <- summary(mod_chls_LFM)
chlsSumm_LFM #Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_chls_LFM)
gam.check(mod_chls_LFM_gaussian)
par(mfrow = c(1, 1)) 

### now transform response variable:
fam<-family(mod_chls_LFM)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
chlsYear_LFM <- with(LFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_LFM on the link scale
# if you do type = "response" se might still be on the link scale
chlsYear_LFM <- cbind(chlsYear_LFM,
                      data.frame(predict(mod_chls_LFM, chlsYear_LFM,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
chlsYear_LFM <- transform(chlsYear_LFM, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

chlsYear_LFM$lake_ID <- 'LFM' # add a column called lake_ID filled with values "LFM"

## Plot fitted trends
ggplot(chlsYear_LFM, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = LFM_short_extrap, mapping = aes(x = year_plot_extrap, y = chls), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_chls_LFM))
fd_inc
...
LFM_chls <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
LFM_diato <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
LFM_allo <- ggplot(LFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
LFM_PI <- ggplot(LFM_short_extrap_PI) +
   #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
LFM_short_extrap_mchl <- LFM_short_extrap %>% drop_na(year_plot_extrap, master_chl)
...
LFM_master_chl <- ggplot(LFM_short_extrap_mchl) +
   #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/LFM_masterPI.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
mod_d15N_UFM_log <- gam(d15N ~ s(year_plot_extrap, k = 5), 
                     family = Gamma(link = "log"), 
                     data = UFM_short_extrap,
                     method = "REML")
...
UFM_perC <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
UFM_perN <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
```{r}
# If the code isn't running might have to rename C:N in UFM_short_extrap
...
```{r}
# If the code isn't running might have to rename C:N in UFM_short_extrap
# UFM_short_extrap <- rename(UFM_short_extrap, C.Nmolar = `C:N`)
names(UFM_short_extrap)

# check later how weights, corCAR1 compare. 
    
  hist(UFM_short_extrap$C.Nmolar)
  hist(log(UFM_short_extrap$C.Nmolar))
  # data is left skewed, log transform makes it worse? in this case it looks a little better?
    
mod_C.Nmolar_UFM <- gam(C.Nmolar ~ s(year_plot_extrap, k = 5), 
                     family = gaussian(link = "identity"), 
                     data = UFM_short_extrap,
                   #  correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")
mod_C.Nmolar_UFM_gamma <- gam(C.Nmolar ~ s(year_plot_extrap, k = 5), 
                     family = Gamma(link = "log"), 
                     data = UFM_short_extrap,
                   #  correlation = corCAR1(form = ~ year_plot_extrap),
                     method = "REML")

AIC(mod_C.Nmolar_UFM, mod_C.Nmolar_UFM_gamma) # gaussian better
summary(mod_C.Nmolar_UFM)

### Check diagnostics. You might need to adjust model
gam.check(mod_C.Nmolar_UFM)

# p value too low if you change anything (add car 1 etc.)

    resid_values_C.Nmolar_UFM <- residuals(mod_C.Nmolar_UFM, type = "response")
    acf(resid_values_C.Nmolar_UFM) 
    # residuals significant, def structured, corCAR1 helps a little but sample size is too small to use it.
    
## summary object for use in document
C.NmolarSumm_UFM <- summary(mod_C.Nmolar_UFM)
C.NmolarSumm_UFM #Gives you the P values, degrees of freedom...
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
C.NmolarYear_UFM <- with(UFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))

# predict fit (response variable) and se for perCYear_UFM on the response scale
C.NmolarYear_UFM <- cbind(C.NmolarYear_UFM,
                      data.frame(predict(mod_C.Nmolar_UFM, C.NmolarYear_UFM,
                                         type="response", se.fit = TRUE)))

# calculate 95% CIs, on the response scale 

C.NmolarYear_UFM <- transform(C.NmolarYear_UFM,
  upper = fit + 2 * se.fit,
  lower = fit - 2 * se.fit)

C.NmolarYear_UFM$lake_ID <- 'UFM' # add a column called lake_ID filled with values "UFM"
     
## Plot fitted trends
ggplot(C.NmolarYear_UFM, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = UFM_short_extrap, mapping = aes(x = year_plot_extrap, y = C.Nmolar), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_C.Nmolar_UFM))
fd_inc
...
pred <- left_join(C.NmolarYear_UFM,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
UFM_C.Nmolar <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/UFM_C.Nmolar.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
UFM_pheo_a <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
```{r}
# compare data distributions
  hist(UFM_short_extrap$chla_pheoa)
  hist(log(UFM_short_extrap$chla_pheoa)) # better!

    # check later how weights, corCAR1 compare. 
    
mod_chla_pheoa_UFM <- gam(chla_pheoa ~ s(year_plot_extrap, k = 5),
                     family=Gamma(link="log"),  
                     data = UFM_short_extrap,
                     method = "REML")


summary(mod_chla_pheoa_UFM) 

resid_values_chla_pheoa_UFM <- residuals(mod_chla_pheoa_UFM, type = "response")
acf(resid_values_chla_pheoa_UFM) # looks good!

## summary object for use in document
chla_pheoaSumm_UFM <- summary(mod_chla_pheoa_UFM)
chla_pheoaSumm_UFM #Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
gam.check(mod_chla_pheoa_UFM)

### now transform response variable:
fam<-family(mod_chla_pheoa_UFM)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 
...
# Make a 200 row list of possible year_plot_extrap values from min to max.
chla_pheoaYear_UFM <- with(UFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_UFM on the link scale
# if you do type = "response" se might still be on the link scale
chla_pheoaYear_UFM <- cbind(chla_pheoaYear_UFM,
                      data.frame(predict(mod_chla_pheoa_UFM, chla_pheoaYear_UFM,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
chla_pheoaYear_UFM <- transform(chla_pheoaYear_UFM, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

chla_pheoaYear_UFM$lake_ID <- 'UFM' # add a column called lake_ID filled with values "UFM"

## Plot fitted trends
ggplot(chla_pheoaYear_UFM, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = UFM_short_extrap, mapping = aes(x = year_plot_extrap, y = chla_pheoa), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_chla_pheoa_UFM))
fd_inc
...
pred <- left_join(chla_pheoaYear_UFM,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
UFM_chla_pheoa <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/UFM_chla_pheoa.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
UFM_diato <- ggplot(UFM_short_extrap) +
  #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
UFM_short_extrap_mchl <- UFM_short_extrap %>% drop_na(year_plot_extrap, master_chl)
...
UFM_master_chl <- ggplot(UFM_short_extrap_mchl) +
   #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
ggsave("figures/UFM_masterPI.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
UFM_PI <- ggplot(UFM_short_extrap_PI) +
   #Add a box to show the reader the time period for interpretation
    geom_rect(
    mapping = aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax), 
    data = highlight_data, 
...
Want to arrange the plot so you have 5 rows (%C, %N, C:N, d15N, d13C) 
then 3 columns (LFM, TRK, UFM)
Use the patchwork R package
```{r}
...
theme_update(
  ...
  legend.text  = element_text(size = 12),   # text inside legend
  legend.title = element_text(size = 14)    # legend title
)


# Row 1 
(LFM_perC + theme(axis.title.x=element_blank(),
                    axis.text.x=element_blank())) +
            (UFM_perC + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank()))  + 
            (TRK_perC + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank())) + 
  
# Row 2
(LFM_perN + theme(axis.title.x=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_perN + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               plot.title = element_blank(),
                     axis.text.x=element_blank())) + 
              (TRK_perN + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               plot.title = element_blank(),
                               axis.text.x=element_blank())) + 
# Row 3
(LFM_C.Nmolar + theme(axis.title.x=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_C.Nmolar + theme(axis.title.y=element_blank(),
                  plot.title = element_blank(),
                  axis.title.x=element_blank(),
                  axis.text.x=element_blank()))  + 
  (TRK_C.Nmolar + theme(axis.title.y=element_blank(),
                 plot.title = element_blank(),
                 axis.text.x=element_blank(), 
                 axis.title.x=element_blank()))  + 
  
#Row 4  
(LFM_d13C + theme(axis.title.x=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_d13C + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) +
  (TRK_d13C + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) + 

#Row 5
(LFM_d15N + theme(axis.title.x=element_blank(),
                    plot.title = element_blank())) +
  (UFM_d15N + theme(axis.title.y=element_blank(),
                    plot.title = element_blank()))  +
  (TRK_d15N + theme(axis.title.y=element_blank(),
                   plot.title = element_blank(),
                   axis.title.x=element_blank())) +

plot_layout(ncol = 3, nrow=5) + 
plot_layout(guides = 'collect') +  # a single legend 
...
ggsave("figures/FigureX_carbon_nitrogen.png",
       dpi=600,
       height=8,
       width=12,
       units="in")
...
Want to arrange the plot so you have 6 rows and 3 columns (LFM, UFM, TRK)
Use the patchwork R package
```{r}
...
# Row 1 
(LFM_chla_pheoa + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    axis.text.x=element_blank())) +
            (UFM_chla_pheoa + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank()))  + 
            (TRK_chla_pheoa + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank())) + 
  
# Row 2
(LFM_greens + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_greens + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               plot.title = element_blank(),
                     axis.text.x=element_blank())) + 
              (TRK_greens + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               plot.title = element_blank(),
                               axis.text.x=element_blank())) + 
# Row 3
(LFM_diato + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_diato + theme(axis.title.y=element_blank(),
                  plot.title = element_blank(),
                  axis.title.x=element_blank(),
                  axis.text.x=element_blank()))  + 
  (TRK_diato + theme(axis.title.y=element_blank(),
                 plot.title = element_blank(),
                 axis.text.x=element_blank(), 
                 axis.title.x=element_blank()))  + 
  
#Row 4  
(LFM_allo + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_allo + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) +
  (TRK_allo + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) + 
  
#Row 5 
(LFM_cyanos + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank())) + 
  (UFM_cyanos + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) +
  (TRK_cyanos + theme(axis.title.y=element_blank(),
                    plot.title = element_blank(),
                    axis.text.x=element_blank(),
                    axis.title.x=element_blank())) + 

#Row 5
(LFM_PI + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    plot.title = element_blank())) +
  (UFM_PI + theme(axis.title.y=element_blank(),
                    plot.title = element_blank()))  +
  (TRK_PI + theme(axis.title.y=element_blank(),
                   plot.title = element_blank(),
                   axis.title.x=element_blank())) +
...
ggsave("figures/FigureY_pigments.png",
       dpi=600,
       height=8,
       width=12,
       units="in")
