...
# compare data distributions
  hist(TRK_short$all_chl)
  hist(log(TRK_short$all_chl)) # better!

    # check later how weights, corCAR1 compare. 
    
 mod_all_chl_TRK <- gam(all_chl ~ s(year_plot_extrap, k = 15),
                     family=Gamma(link="log"),  
                     data = TRK_short,
                     method = "REML")


summary(mod_all_chl_TRK) 

resid_values_all_chl_TRK <- residuals(mod_all_chl_TRK, type = "response")
acf(resid_values_all_chl_TRK) 

## summary object for use in document
all_chlSumm_TRK <- summary(mod_all_chl_TRK)
all_chlSumm_TRK # Gives you the P values, degrees of freedom...

### Check diagnostics. You might need to adjust model
par(mfrow = c(2, 2))   # Set up a 2x2 grid
gam.check(mod_all_chl_TRK)
par(mfrow = c(1, 1)) 

### now transform response variable:
fam<-family(mod_all_chl_TRK)
fam # this is a list of 13
ilink <- fam$linkinv
ilink 

plot(mod_all_chl_TRK)
### NOW PLOT! ###
N <- 300    # number of points at which to evaluate the splines

# Make a 200 row list of possible year_plot_extrap values from min to max.
all_chlYear_TRK <- with(TRK_short, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), 
                                                   max(year_plot_extrap, na.rm=TRUE), 
                                                   length.out = 200)))
...
# predict fit (response variable) and se for perCYear_TRK on the link scale
# if you do type = "response" se might still be on the link scale
all_chlYear_TRK <- cbind(all_chlYear_TRK,
                      data.frame(predict(mod_all_chl_TRK, all_chlYear_TRK,
                                         type="link", se.fit = TRUE)))

# calculate 95% CIs, then transform back to the response scale 
all_chlYear_TRK <- transform(all_chlYear_TRK, fit   = ilink(fit),
  upper = ilink(fit + 2 * se.fit),
  lower = ilink(fit - 2 * se.fit))

all_chlYear_TRK$lake_ID <- 'TRK' # add a column called lake_ID filled with values "TRK"

## Plot fitted trends
ggplot(all_chlYear_TRK, aes(x = year_plot_extrap, y = fit)) + # year_plot_extrap is the interp time var
geom_ribbon(aes(ymin = (lower),
                ymax = (upper), x = year_plot_extrap),
            alpha = 0.2, inherit.aes = FALSE, fill = "black") +
      geom_point(data = TRK_short, mapping = aes(x = year_plot_extrap, y = all_chl), inherit.aes = FALSE) +
      geom_line() 
...
# all these are calculate on the link scale log(mu) because the model specifies the gamma family
fd_inc = confint(fderiv(mod_all_chl_TRK))
fd_inc
...
pred <- left_join(all_chlYear_TRK,
                  fd_inc_timing %>% 
                    select(year_plot_extrap, change_type), #select only 2 columns since
                  # lower and upper have two different meanings in these 2 dataframes
                  by="year_plot_extrap") 
...
TRK_all_chl <- ggplot(TRK_short) +
  #Add a box to show the reader the time period for interpretation
    ...
    fill = "blue",
    inherit.aes = FALSE) + 
  geom_point(aes(year_plot_extrap, all_chl)) +
  geom_ribbon(data = pred,
              aes(x = year_plot_extrap, ymin = lower, ymax = upper),
              alpha = 0.2) +
  geom_line(data = pred,
...
#            linewidth = 1) +
  scale_color_manual(values = c("sig. inc." = "#CC9933",
                     ...
                     name ="Statistic") +
  # scale_x_continuous(breaks=seq(1880,2025,by=25))+
  ylab(expression("total chlorophylls ("*nmol~g^-1~"total carbon"*")")) + 
  xlab(expression(paste("Year (", italic("CE"), ")"))) +
  ggtitle("TRK")
TRK_all_chl

ggsave("figures/TRK_all_chl.png",
       dpi=600,
       height=5,
       width=8,
       units="in")
...
# Row 1 
(LFM_pheo_a + theme(axis.title.x=element_blank(),
                    axis.title.y=element_blank(),
                    axis.text.x=element_blank())) +
            (UFM_pheo_a + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank()))  + 
            (TRK_pheo_a + theme(axis.title.y=element_blank(),
                               axis.title.x=element_blank(),
                               axis.text.x=element_blank())) + 
