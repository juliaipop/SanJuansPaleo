...
#I also copied the first derivatives functions into a separate R script that we can source here
#for Cale's code to run
source("scripts/helper_functions.R")

theme_SJ <- function () {
  theme_base(base_size=12, base_family="PT Sans") %+replace% # base size scales everything
    theme(
      ...
      plot.subtitle = element_text(color="dimgrey", hjust=0, size=12), # hjust = left aligned
      axis.title=element_text(face="plain"), # face = plain means not bold
      panel.grid.minor=element_blank(), # removes minor grid lines
      strip.text.x = element_text(face="italic"),
      panel.grid.major=element_line(color="lightgrey"))}

theme_set(theme_SJ())  # pre-set the bw theme
```
