...
resid_values_nw <- residuals(mod_d15N_LFM_nw, type = "normalized") # no CAR(1), no weights
...
### Try again but with extrapolated dates this time!
```{r}
### K (sets the maximum # of bases for our spline): 
...
mod_d15N_LFM_ex <- gamm(d15N ~ s(year_plot_extrap, k = 10), 
                     family = gaussian(link = "identity"), 
                     # should I use weights here?                     
                     # check with bella
                     data = LFM_short_extrap,
                     method = "REML")

mod_d15N_LFM_ex_CAR1 <- gamm(d15N ~ s(year_plot_extrap, k = 10), 
                     family = gaussian(link = "identity"), 
                     # does family correspond to the data or their residuals? 
                     # check with bella
                     data = LFM_short_extrap,
...
resid_values_ex_CAR1 <- residuals(mod_d15N_LFM_ex_CAR1$lme, type = "normalized")   # with CAR(1)
all.equal(resid_values_ex, resid_values_ex_CAR1)  # Check if they are identical
# not the same

acf(resid_values_ex)
acf(resid_values_ex_CAR1)

# examine again using AIC:
AIC(mod_d15N_LFM_ex$lme)   # without CAR(1): -19.76936
AIC(mod_d15N_LFM_ex_CAR1$lme) # with CAR(1): -28.97341
# lower AIC = better? better with CAR(1) still. 
# AIC=2k−2ln(L), where k = # parameters, L = likelihood.


# check phi in the CAR1 gam
d15N_LFM_ex_CAR1_phi <- intervals(mod_d15N_LFM_ex_CAR1$lme)$corStruct
d15N_LFM_ex_CAR1_phi
# ^was rly low earlier when we were modeling just the interpolated dates.
# higher now but has huge confidence intervals
...
### Do it again for the other model...
d15NYear_LFM_ex_CAR1 <- with(LFM_short_extrap, 
                     data.frame(year_plot_extrap = seq(min(year_plot_extrap, na.rm=TRUE), max(year_plot_extrap, na.rm=TRUE), length.out = 200)))

d15NYear_LFM_ex_CAR1 <- cbind(d15NYear_LFM_ex_CAR1,
                      data.frame(predict(mod_d15N_LFM_ex_CAR1$gam, d15NYear_LFM_ex_CAR1,
                                         type="response", se.fit = TRUE)))

### this calculates on the link scale 
d15NYear_LFM_ex_CAR1 <- transform(d15NYear_LFM_ex_CAR1, upper = fit + (2 * se.fit), lower = fit - (2 * se.fit))
d15NYear_LFM_ex_CAR1$lake_ID <- 'LFM'

## Plot fitted trends
ggplot(d15NYear_LFM_ex_CAR1, aes(x = year_plot_extrap, y = fit)) +
  geom_ribbon(aes(ymin = (lower),
                  ymax = (upper), x = year_plot_extrap),
              alpha = 0.2, inherit.aes = FALSE, fill = "black") +
  geom_point(data = LFM_short_extrap, mapping = aes(x = year_plot_extrap, y = d15N), inherit.aes = FALSE) +
